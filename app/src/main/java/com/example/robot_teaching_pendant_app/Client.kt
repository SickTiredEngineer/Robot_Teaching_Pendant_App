        ...
        try {
            ...
            if(mainCommandType == 0x00.toByte()){ //MainCommandType Global
                if (subCommandType == 0x00.toByte()) {
                    ...
                    buffer.putFloat(RobotPosition.global_x)
                    buffer.putFloat(RobotPosition.global_y)
                    buffer.putFloat(RobotPosition.global_z)
                    buffer.putFloat(RobotPosition.global_Rx)
                    buffer.putFloat(RobotPosition.global_Ry)
                    buffer.putFloat(RobotPosition.global_Rz)
                ...
                if (subCommandType == 0x00.toByte()) {
                    ...
                    buffer.put(mainCommandType)
                    buffer.put(subCommandType)
                    buffer.putFloat(RobotPosition.local_x)
                    buffer.putFloat(RobotPosition.local_y)
                    buffer.putFloat(RobotPosition.local_z)
                    buffer.putFloat(RobotPosition.local_Rx)
                    buffer.putFloat(RobotPosition.local_Ry)
                    buffer.putFloat(RobotPosition.local_Rz)
                } else if (subCommandType == 0x01.toByte()) {
                    // subCommandType이 0x01인 경우
                    buffer = ByteBuffer.allocate(39) // 바이트 크기 조정
                    buffer.order(ByteOrder.LITTLE_ENDIAN)
                ...
                if (subCommandType == 0x00.toByte()) {
                    ...
                    buffer.put(mainCommandType)
                    buffer.put(subCommandType)
                    buffer.putFloat(RobotPosition.user_x)
                    buffer.putFloat(RobotPosition.user_y)
                    buffer.putFloat(RobotPosition.user_z)
                    buffer.putFloat(RobotPosition.user_Rx)
                    buffer.putFloat(RobotPosition.user_Ry)
                    buffer.putFloat(RobotPosition.user_Rz)
                } else if (subCommandType == 0x01.toByte()) {
                    // subCommandType이 0x01인 경우
                    buffer = ByteBuffer.allocate(39) // 바이트 크기 조정
                    buffer.order(ByteOrder.LITTLE_ENDIAN)
